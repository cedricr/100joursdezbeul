...
import {detectTargets, detectActions} from 'scripts/updateData/extractAttacData/transformers/detectTargetsAndActions.js';

describe('detecting the targets', () => {
  it('should detect the president', () => {
    expect(detectTargets({'president-rep': ['macron']},[{
  ...
  it('should detect the president anywhere in the alias array', () => {
    expect(detectTargets({'president-rep': ['plop', 'plip', 'macron', 'plup']},[{
      description: 'Macron demission!'
    }])).toEqual([
      {
        description: 'Macron demission!',
  ...
  it('should detect a minister', () => {
    expect(detectTargets({'ministre': ['darmanin']}, [{
  ...
  it('should detect the parliament president', () => {
    expect(detectTargets({'presidente-an': ['braun-pivet']},[{
  ...
  it('should detect the state secretaries', () => {
    expect(detectTargets({'secretaire-etat': ['hairy']},[{
  ...
  it('should detect the delegated ministers', () => {
    expect(detectTargets({'ministre-del': ['klein']},[{
  ...
  it('should detect the prime minister', () => {
    expect(detectTargets({'premiere-min': ['borne']},[{
  ...
  it('should not detect when a target is part of a word', () => {
    expect(detectTargets({'president-rep': ['borne']},[{
...
describe('detecting the actions', () => {
  it('should detect cancelled action from the description', () => {
    expect(detectActions({'annulation': ['annulation']},[{
  ...
  it('should detect sobriete action from the description', () => {
    expect(detectActions({'sobriete': ['sobriété']},[{
  ...
  it('should detect action-creative action from the description', () => {
    expect(detectActions({'creatif': ['opération escargot']},[{
  ...
  it('should detect manif action from the description', () => {
    expect(detectActions({'manif': ['mobilisation']},[{
  ...
  it('should detect chahut action from the description', () => {
    expect(detectActions({'chahut': ['casserolade']},[{
  ...
  it('should detect all the actions together', () => {
    ...
    }])[0].actions).toEqual(['annulation', 'sobriete', 'creatif', 'manif', 'chahut']);
  });
